...
type NavbarButtonOwnProps = {
  href?: string;
  className?: string;
  variant?: "primary" | "secondary" | "dark" | "gradient" | "empty";
  children: React.ReactNode;
};

type NavbarButtonProps<T extends React.ElementType> =
  NavbarButtonOwnProps &
  Omit<React.ComponentPropsWithoutRef<T>, keyof NavbarButtonOwnProps | "as"> & {
    as?: T;
  };
...
export const NavbarButton = <T extends React.ElementType = "a">({
  ...
  const variantStyles: Record<
    "primary" | "secondary" | "dark" | "gradient" | "empty",
    string
  > = {
    primary:
  ...
  return (
    <Tag
      ...
      {...(props as any)} // safe cast: props are already narrowed by Omit
    >
      {children}
    </Tag>
  );
